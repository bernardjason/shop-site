...
class CheckoutTestCase(TestCase):
    def setUp(self):
        ...
        self.stockToAdd = Stock.objects.create(stockId='abc456',description="add to basket",name="helloworld",numberOf=5)
    ...
    def test_checkout(self):
        """checkout basket"""
        item1 = {'name': self.stock.name, 'numberOf': 2, 'description': self.stock.description, 'key':self.stock.stockId, 'stockId': self.stock.stockId}
        item2 = {'name': self.stockToAdd.name, 'numberOf': 2, 'description': self.stockToAdd.description,  'key':self.stockToAdd.id,'stockId': self.stockToAdd.stockId}
        request_data = {
            "basketId":"hello-1",
            "userId":"hello",
        ...
        self.assertEquals({'basketId': 'hello-1', 'message': 'Dispatched', 'address': {'line1': 'the house', 'line2': 'the postcode'}, 'items': []},
            json_data)
